...
function playerListAjax(page, sortColumn, sortType, idx) { // page, sortColumn, sortType, idx
    const innerHtml = $("#playerListAjax")
    ...
    //list.html 에서 id가 searchType인 애의 값을 가져와서 변수 select_value에 넣는다.
...
function sortUpDown(idx) {
    ...
    if ($('#sortType').val() === 'DESC' && imgTag.className === $('#sortColumn').val()) {
        playerListAjax('1', imgTag.className, 'ASC', idx);
    } else {
        playerListAjax('1', imgTag.className, 'DESC', idx);
    }
}
